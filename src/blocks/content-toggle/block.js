 ...
 */
registerBlockType('gutenbergpack/content-toggle', {
	...
	attributes: {
		accordions: {
			source: 'query',
			selector: '.wp-block-gutenbergpack-content-toggle-accordion',
			query: {
				title: { type: 'array', source: 'children', selector: '.wp-block-gutenbergpack-content-toggle-accordion-title' },
				content: { type: 'array', source: 'children', selector: '.wp-block-gutenbergpack-content-toggle-accordion-content' },
			},
		},
		accordionsState: {
			type: 'string',
			...
			<div>
				{
					accordions.map((accordion, i) => {
						return <div style={{ borderColor: props.attributes.theme }} className="wp-block-gutenbergpack-content-toggle-accordion" key={i}>
							<div className="wp-block-gutenbergpack-content-toggle-accordion-title-wrap" style={{ backgroundColor: props.attributes.theme }}>
								<span
									className="wp-block-gutenbergpack-content-toggle-accordion-title"
									style={{
										color: props.attributes.titleColor
									}}>
									{accordion.title}
								</span>
								<span className={'wp-block-gutenbergpack-content-toggle-accordion-state-indicator dashicons dashicons-arrow-right-alt2 ' + (collapsed ? '' : 'open')}></span>
							</div>
							<div style={{ display: (collapsed ? 'none' : 'block') }} className="wp-block-gutenbergpack-content-toggle-accordion-content-wrap">
								<div className="wp-block-gutenbergpack-content-toggle-accordion-content">{accordion.content}</div>
							</div>
						</div>;
					})
				}
