...
function gutenbergpack_render_progress_bar_block($attributes){
    $elementID = generateRandomString();
    $chosenProgressBar = '';
    $initializationCode = '';
    $className = 'gutenbergpack_progress-bar';
    ...
    return '<div class="gutenbergpack_progress-bar">
                <div class="gutenbergpack_progress-bar-text">
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            let label = container.getElementsByClassName("gutenbergpack_progress-bar-label")[0];
            '. $initializationCode
...
function gutenbergpack_register_progress_bar_block() {
	if( function_exists( 'register_block_type' ) ) {
		register_block_type( 'gutenbergpack/progress-bar', array(
            'attributes' => array(
                'percentage' => array(
                    'type' => 'number',
                    'default' => 25
            ...
            'render_callback' => 'gutenbergpack_render_progress_bar_block'));
    }
}

add_action( 'init', 'gutenbergpack_register_progress_bar_block' );