 ...
 */
function gutenbergpack_register_meta() {
	register_meta( 'post', 'gutenbergpack_ctt_tweet', array(
		'show_in_rest' => true,
		'single' => true
	) );
	register_meta( 'post', 'gutenbergpack_ctt_via', array(
		'show_in_rest' => true,
		'single' => true
	) );
}

add_action( 'init', 'gutenbergpack_register_meta' );
...
function render_click_to_tweet_block( $attributes ) {
	...
	$via = get_post_meta( get_the_ID(), 'gutenbergpack_ctt_via', true );
	$via = ( $via ) ? '&via=' . str_replace( '@', '', $via ) : false;
	$tweet      = get_post_meta( get_the_ID(), 'gutenbergpack_ctt_tweet', true );
	...
	$url       = apply_filters( 'gutenbergpack_click_to_tweet_url', "http://twitter.com/share?&text={$tweet_url}&url={$permalink}{$via}" );

	$output = '';
	$output .= sprintf('<div class="gutenbergpack_click_to_tweet" style="%1$s">', esc_attr( $borderColor ));
	$output .= sprintf( '<div class="gutenbergpack_tweet" style="%1$spx; %2$s"> %3$s', esc_attr( $tweetFontSize ), esc_attr( $tweetColor ), esc_html( $tweet ) );
	$output .= sprintf('</div>');
	$output .= sprintf( '<div class="gutenbergpack_click_tweet">' );
...
function register_block() {
	if ( function_exists( 'register_block_type' ) ) {
		register_block_type( 'gutenbergpack/click-to-tweet', array(
