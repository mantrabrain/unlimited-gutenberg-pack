 ...
 */
registerBlockType('gutenbergpack/click-to-tweet', {
	...
	attributes: {
		ubTweet: {
			source: 'meta',
			meta: 'gutenbergpack_ctt_tweet'
		},
		ubVia: {
			source: 'meta',
			meta: 'gutenbergpack_ctt_via'
		},
		tweetFontSize: {
			type: 'number',
			default: 20
	...
	edit: function (props) {
		...
		return [
			...
			<div className={props.className}>
				<div
					className="gutenbergpack_click_to_tweet"
					style={{
				...
				>
					<TextareaAutosize
						...
						className="gutenbergpack_tweet"
						value={ubTweet}
						onChange={(event) => { props.setAttributes({ ubTweet: event.target.value }) }}
						focus={props.focus}
						keepPlaceholderOnFocus={true}
					/>
					<div className="gutenbergpack_click_tweet">
						<span
							style={{
								display: 'inline-flex'
							}}
